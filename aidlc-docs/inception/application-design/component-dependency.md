...
| BE-04 CharacterLambda | DynamoDB（CharacterDialogues） | AWS SDK | キャッシュ台詞取得 |
...
features/recipe
  ...
  +--(6) trigger=recipe_registered --> BE-04: CharacterLambda
                                          |
                                          +--(7) キャッシュ検索 --> DynamoDB（CharacterDialogues）※BE-04所有
                                          <--(8) キャラクター・台詞--
  |
  <--(9) ボトムシート表示--
```
...
### ガチャフロー
```
features/gacha（レアリティ抽選はフロントエンドで実行）
  |
  +--(1) ガチャ実行リクエスト --> BE-03: GachaLambda
                                    |
                                    ...
                                    |                        +--> DynamoDB（Recipes）※BE-01所有
                                    |                        <-- レシピリスト--
                                    +--(4) 確定済み献立を除外してレシピリストを返却
  |
  <--(5) 除外済みレシピリスト返却--
  |
  +--(6) フロントエンドでレアリティ抽選（N:60%/R:25%/SR:12%/SSR:3%）
  +--(7) 結果表示・ユーザーが確定を選択
  |
  +--(8) 確定済み献立登録 --> BE-02: MenuSuggestionLambda（フロントエンドから直接呼び出し）
